...
cc_library(
    ...
    srcs = glob([
        ...
        "third_party/intel/lib/Dialect/TritonGEN/IR/*.cpp",
        "third_party/intel/lib/Dialect/TritonIntelGPU/IR/*.cpp",
    ...
    hdrs = glob([
        ...
        "third_party/intel/include/Dialect/TritonGEN/IR/*.h",
        "third_party/intel/include/Dialect/TritonIntelGPU/IR/*.h",
        # There are so many interdependencies between Dialect and Analysis that we're just compiling
        # everything in a single unit.
...
cc_library(
    ...
    hdrs = glob(
        ...
        # we need to include them here to avoid circular dependency.
        "include/triton/Dialect/Triton/Transforms/LoopPeeling.h",
        "lib/Dialect/TritonGPU/Transforms/WarpSpecialization/PartitionAttrs.h",
    ],
    copts = select({
        ":compiler_is_msvc": [],
        "//conditions:default": [
