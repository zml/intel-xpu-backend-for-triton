...
def test_dot_fma(device):
    ...
    b = torch.rand((B, B), dtype=torch.float32, device=device)
...
def test_split_auto_layout_execution(device):
    threads_per_warp = ttgl.constexpr(THREADS_PER_WARP)
    ...
    XBLOCK = 128
    input = torch.randint(-100, 100, (2 * XBLOCK, ), device=device, dtype=torch.int32)
    output = torch.empty(XBLOCK, device=device, dtype=torch.int32)
...
@pytest.mark.xfail(not is_blackwell_ultra(), reason="Requires Blackwell Ultra", run=False)
