...
from triton._internal_testing import (is_blackwell, is_cuda, is_hip, is_hip_cdna3, is_hip_cdna4, is_hip_gfx1250,
                                      is_hopper, is_interpreter, is_xpu)
...
def test_cossin_angle_identities(device, mode, fresh_knobs):
    ...
    x = torch.randint(-(2**31), 2**31 - 1, (n_elements, ), dtype=torch.int32, device=device, generator=g)
    y = torch.randint(-(2**31), 2**31 - 1, (n_elements, ), dtype=torch.int32, device=device, generator=g)
    lhs = torch.empty((n_elements, ), dtype=torch.int32, device=device)
    rhs = torch.empty((n_elements, ), dtype=torch.int32, device=device)
...
def test_cast_trunc_ext_payload_semantics(device, fresh_knobs):
    ...
    x[:3] = torch.tensor(special_f32_bits, dtype=torch.int32, device=device)
    out = torch.empty((n_elements, ), dtype=torch.int32, device=device)
...
def test_cast_ext_payload_semantics(device, fresh_knobs):
    ...
    special_f32_bits = np.asarray([-1.0, 0.0, 1.0], dtype=np.float32).view(np.int32)
    x[:3] = torch.tensor(special_f16_bits, dtype=torch.int16, device=device)
    out = torch.empty((n_elements, ), dtype=torch.int32, device=device)
...
@pytest.mark.xfail(not (is_hip_cdna4() or is_hip_gfx1250()), reason="Requires DotScaledOp support (CDNA4, or GFX1250)",
                   run=False)
