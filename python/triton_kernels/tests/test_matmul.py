...
def _test_op(m, n, k, split_k, do_gather, do_scatter, inner_expt_opt, do_gamma, is_persistent, num_warps, n_slices,
    ...
    device_capability = torch.cuda.get_device_capability()[0] if is_cuda() else 0
    ...
    if a_hbm_swizzling:
        ...
        if not a_dtype.has_mx_scale:
            pytest.xfail(f"NYI. X swizzling only implemented for microscaled activations for now. Got {act_dtype_str}")
        ...
        if do_gather:
            pytest.xfail("X swizzling does not support gathered activations")

    if c_hbm_swizzling:
        if is_hip() or (is_cuda() and device_capability < 10):
    ...
    if is_cuda() and b_dtype.is_any_float8 and device_capability < 10:
        b_transpose = True
    ...
    if shuffle_mxfp4_w_layout:
        ...
        if is_hip() or not is_cuda() or device_capability < 10:
            pytest.xfail("Shuffled MXFP4 weight layout requires Blackwell or newer")
        if b_dtype.name != "mxfloat4_e2m1":
